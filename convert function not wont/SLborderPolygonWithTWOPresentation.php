...
$data1 = $hello->get_data($tbl);
$data2 = $hello->get_data($tbl2);
$data3 = $hello->get_data($tbl3);
...
<script>
    ...
    function initMap() {
        ...
        var google_path = JSON.parse('<?php echo $data; ?>');
		var google_path = JSON.parse('<?php echo $data; ?>');
		var google_path = JSON.parse('<?php echo $data; ?>');
        var boatLatLongASInput = new google.maps.LatLng (google_path[0]);
       ...
       //  var boatLatLongASInput = new google.maps.LatLng ({ "lat":9.099491   ,  "lng":  79.533390 });   //  within two borders :-lat=9.099131    long=79.537861
        ...
        if( google.maps.geometry.poly.containsLocation(  boatLatLongASInput, SLLatLonForFunction) == true ) {
            ...
            if (google.maps.geometry.poly.containsLocation(boatLatLongASInput, LatLonForFunctionYuCreated) == true) {
                ...
                for (var i2 = 0; i < google_path.length; i++) {
                ...
                for (i2 = 0; i2 < SLOuterBorder.length; i2++) {
                    var SLorderLatLng = new google.maps.LatLng(SLOuterBorder[i]);
                    All_Distencec2[i] = google.maps.geometry.spherical.computeDistanceBetween(SLorderLatLng, boatLatLongASInput);
                }
            ...
            /*************************************sitina thana pennweema**********************************************/
            for (var i2 = 0; i2 < google_path.length; i2++) {
            ...
            for (i2 = 0; i2 < SLOuterBorder.length; i2++) {
                ...
                All_Distencec2[i2] = google.maps.geometry.spherical.computeDistanceBetween(SLorderLatLng, boatLatLongASInput);
            }
