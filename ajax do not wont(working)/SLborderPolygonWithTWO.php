...
<script>
    ...
    function initMap() {
        ...
        var SLOuterBorderOnMap = new google.maps.Polygon({
            paths: SLOuterBorder,
            strokeColor: '#FF0000',
            strokeOpacity: 0.8,
            strokeWeight: 2.0,
        ...
        var boatLatLongASInput = [];
        var j;

        for (j = 0; j < 3; j++) {


            var google_path = JSON.parse('<?php echo $data[$j]; ?>');
            alert(google_path);

            var boatLatLongASInput[j] = new google.maps.LatLng(google_path[0]);


            // var boatLatLongASInput = new google.maps.LatLng ( { lat:9.377270  , lng:  80.001412});   //  within border you created  :- lat=9.075000 long=80.393144
            //  var boatLatLongASInput = new google.maps.LatLng ({ "lat":9.099491   ,  "lng":  79.533390 });   //  within two borders :-lat=9.099131    long=79.537861
            //  var boatLatLongASInput = new google.maps.LatLng ( { lat:9.535773 , lng:79.107743 });   // croos the sri lanken border  :- lat=9.535773 long=79.107743


            x = google.maps.geometry.poly.containsLocation(boatLatLongASInput[j], LatLonForFunctionYuCreated);
            y = google.maps.geometry.poly.containsLocation(boatLatLongASInput[j], SLLatLonForFunction);
            /*====================================================================================================================================*/
            /*===================================================================================================================================*/
            ...
            if (google.maps.geometry.poly.containsLocation(boatLatLongASInput[j], SLLatLonForFunction) == true) {

                if (google.maps.geometry.poly.containsLocation(boatLatLongASInput[j], LatLonForFunctionYuCreated) == true) {
                ...
                else if (google.maps.geometry.poly.containsLocation(boatLatLongASInput[j], LatLonForFunctionYuCreated) == false) {
                    ...
                    for (i = 0; i < SLOuterBorder.length; i++) {
                        ...
                        All_Distencec[i] = google.maps.geometry.spherical.computeDistanceBetween(SLorderLatLng, boatLatLongASInput[j]);
                    }
            ...
            else if (google.maps.geometry.poly.containsLocation(boatLatLongASInput[j], SLLatLonForFunction) == false) {
